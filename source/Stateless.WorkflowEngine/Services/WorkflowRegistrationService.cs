...
namespace Stateless.WorkflowEngine.Services
{
    public interface IWorkflowRegistrationService
    {
        ...
        /// <typeparam name="T"></typeparam>
        /// <returns></returns>
        Task<bool> IsSingleInstanceWorkflowRegisteredAsync<T>(IWorkflowStore workflowStore) where T : Workflow;
        ...
        /// </summary>
        /// <param name="workflow">The workflow.</param>
        Task RegisterWorkflowAsync(IWorkflowStore workflowStore, Workflow workflow);
    }
    ...
    public class WorkflowRegistrationService : IWorkflowRegistrationService
    {
        ...
        {
            IEnumerable<T> workflows = workflowStore.GetAllByQualifiedName<T>();
        ...
        /// <typeparam name="T"></typeparam>
        /// <returns></returns>
        public async Task<bool> IsSingleInstanceWorkflowRegisteredAsync<T>(IWorkflowStore workflowStore) where T : Workflow
        {
            IEnumerable<T> workflows = await workflowStore.GetAllByQualifiedNameAsync<T>();
            T wf = workflows.SingleOrDefault();
            if (wf == null) return false;
            if (!wf.IsSingleInstance) throw new WorkflowException(String.Format("A workflow of type {0} is registered, but not as a single instance workflow.", typeof(T).FullName));
            return true;
        ...
        /// </summary>
        /// <param name="workflow">The workflow.</param>
        public async Task RegisterWorkflowAsync(IWorkflowStore workflowStore, Workflow workflow)
        {
            if (workflow.IsSingleInstance)
            {
                string qualifiedName = workflow.QualifiedName;
                IEnumerable<Workflow> workflows = await workflowStore.GetAllByQualifiedNameAsync(qualifiedName);
                if (workflows.Any())
                {
                    throw new SingleInstanceWorkflowAlreadyExistsException(String.Format("Workflow of type '{0}' already registered", qualifiedName));
                }
